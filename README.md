...
- **压缩导出**：默认以流式 gzip 压缩为 `.tar.gz`（典型节省 50%~70% 存储与传输体积），目标机 `docker load -i xxx.tar.gz` **可直接加载、无需先解压**；侧边栏开关可随时切回未压缩 `.tar`。
- **可中止任务执行**：运行中可一键中止，界面状态与后台 Docker 子进程保持一致，不会“界面中止但后台继续”。
- **实时日志与进度条**：使用 PTY 伪终端技术，完美捕获并展示 Docker 命令的层级（Layer）下载进度。
- **导出历史**：成功导出自动落档（上限 200 条，重启不丢失），支持打开所在文件夹、单条删除与一键清空（需确认，不影响 tar 文件）。
- **本机镜像管理**：查看本机已下载镜像（名称/创建时间/体积）、搜索过滤、勾选批量或单个删除（均有二次确认），便于释放磁盘空间。
...
> 💡 **目标机导入**：现代 Docker 均支持直接加载压缩归档——`docker load -i 镜像名_tag_平台.tar.gz`，无需先解压。
