...
2. **安装核心依赖**：
   ...
   ```
   当前 `requirements.txt` 固定 `flet==0.81.0` 以保证界面行为与打包结果一致。
   另外，项目依赖本机 Docker CLI（`docker` 命令），不依赖 Python Docker SDK。
...
若显示 `✗ 未安装` 或 `✗ 未运行`，按提示安装/启动 Docker Desktop 后重新检查。
应用界面侧边栏底部的「环境状态」卡片提供同样的实时状态（含手动刷新按钮）。
